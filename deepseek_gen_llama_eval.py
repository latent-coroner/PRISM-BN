"""
llm_judge_deepseek_llama4pass.py — 4-pass Bayesian Network extraction & evaluation
...
Extraction model : Extraction Model       (via HuggingFace InferenceClient)
...
def call_llm(client, system, user, model, max_tokens=2000):
...
def kl_div(p, q):
    ...
    return sum(pi * math.log(pi/qi) for pi, qi in zip(p, qn) if pi > 0 and qi > 0)
...
def col_kl(pred_matrix, gt_matrix, n_gt_parent_states, n_gt_child_states):
    ...
    for j in range(n_gt_parent_states):
        if j >= nc:
            break
        gt_col = [float(gt_matrix[r][j]) if r < len(gt_matrix) and j < len(gt_matrix[r]) and gt_matrix[r][j] is not None else 0.0
...
def process_subgraph(sg_id, sg, client):
    ...
    print("    Pass 1: nodes...")
    p1 = call_llm(client, PASS1_SYSTEM, pass1_prompt(text), EXTRACTION_MODEL,
    ...
    nj = call_llm(client, NODE_JUDGE_SYSTEM,
                  node_judge_prompt(gt_names, pred_names), JUDGE_MODEL,
    ...
    # ── Pass 2: extract states for predicted node names ───────────────────────
    print("    Pass 2: states...")
    ...
    p2 = call_llm(client, PASS2_SYSTEM,
                  pass2_prompt(text, all_pred_for_p2), EXTRACTION_MODEL,
    ...
    for pred_name, gt_name in node_match_map.items():
        ...
        sj = call_llm(client, STATE_JUDGE_SYSTEM,
    ...
    # ── Pass 3: extract edges using canonical predicted node names ────────────
    print("    Pass 3: edges...")
    ...
    p3 = call_llm(client, PASS3_SYSTEM,
                  pass3_prompt(text, node_states_for_prompt), EXTRACTION_MODEL,
    ...
    # ── Pass 4: CPD matrix for each correctly predicted edge ──────────────────
    ...
    for (gt_parent, gt_child) in correct_edges:
        ...
        p4 = call_llm(client, PASS4_SYSTEM,
...
def main():
    ...
    ap.add_argument("--output-csv",      default="anonymized_eval_results.csv")
