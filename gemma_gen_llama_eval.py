"""
llm_judge_gemma_llama.py — 4-pass Bayesian Network extraction & evaluation
...
Extraction model : Gemma 3 12B   (via HuggingFace InferenceClient / featherless-ai)
...
def call_gemma(client, system, user, max_tokens=2000):
    """Extraction calls — Gemma via novita (requires multimodal content format)."""
...
def eval_pass1(judge_result, n_gt, n_pred):
    valid = [m for m in judge_result.get("matches", [])
...
def process_subgraph(sg_id, sg, client):
    ...
    # ── Pass 1: extract nodes (Gemma) ────────────────────────────────────────
    print("    Pass 1: nodes...")
    p1 = call_gemma(client, PASS1_SYSTEM, pass1_prompt(text),
    ...
    # Judge nodes (Llama)
    nj = call_llama(client, NODE_JUDGE_SYSTEM,
    ...
    # ── Pass 2: extract states for predicted node names (Gemma) ──────────────
    print("    Pass 2: states...")
    ...
    p2 = call_gemma(client, PASS2_SYSTEM,
    ...
    for pred_name, gt_name in node_match_map.items():
        ...
        # Judge states (Llama)
        sj = call_llama(client, STATE_JUDGE_SYSTEM,
    ...
    # ── Pass 3: extract edges using canonical predicted node names (Gemma) ────
    print("    Pass 3: edges...")
    ...
    p3 = call_gemma(client, PASS3_SYSTEM,
    ...
    # ── Pass 4: CPD matrix for each correctly predicted edge (Gemma) ──────────
    ...
    for (gt_parent, gt_child) in correct_edges:
        ...
        p4 = call_gemma(client, PASS4_SYSTEM,
...
def run_result_analysis(results, out_dir="gemma_anonymized_result_analysis"):
...
def main():
    ...
    ap.add_argument("--output-analysis", default="gemma_gen_llama_eval_result_analysis")
