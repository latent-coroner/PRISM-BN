"""
llm_judge_gpt4o_llama.py — 4-pass Bayesian Network extraction & evaluation
...
Extraction model : Extraction Model       (via OpenAI API)
...
Scoring: 1=same meaning or clear paraphrase, 0=no meaningful match.
...
def call_gpt4o(client, system, user, max_tokens=2000):
    """Extraction calls — Extraction Model via OpenAI API."""
...
def call_llama(client, system, user, max_tokens=2000):
    """Judge calls — Judge Model via Groq (plain string content)."""
    for attempt in range(MAX_RETRIES):
        try:
            completion = client.chat.completions.create(
                model=JUDGE_MODEL,
...
def kl_div(p, q):
    ...
    qs = [v + LAPLACE_EPS for v in q]; qn = [v/sum(qs) for v in qs]
...
def col_kl(pred_matrix, gt_matrix, n_gt_parent_states, n_gt_child_states):
    ...
    for j in range(n_gt_parent_states):
        if j >= nc:
            break
        gt_col = [float(gt_matrix[r][j]) if j < len(gt_matrix[r]) and gt_matrix[r][j] is not None else 0.0
...
def eval_pass1(judge_result, n_gt, n_pred):
    valid = [m for m in judge_result.get("matches", []) if m.get("score", 0) > 0]
...
def eval_pass2_node(judge_result, gt_states, pred_states):
    valid  = [m for m in judge_result.get("state_matches", []) if m.get("score", 0) > 0]
    n_corr = len(valid)
...
def process_subgraph(sg_id, sg, openai_client, judge_client):
    result = {
    ...
    # ── Pass 1: extract nodes (Extraction Model) ───────────────────────────────────────
    print("    Pass 1: nodes...")
    p1 = call_gpt4o(openai_client, PASS1_SYSTEM, pass1_prompt(text),
    ...
    # Judge nodes (Llama)
    nj = call_llama(judge_client, NODE_JUDGE_SYSTEM,
    ...
    # ── Pass 2: extract states for predicted node names (Extraction Model) ─────────────
    print("    Pass 2: states...")
    ...
    p2 = call_gpt4o(openai_client, PASS2_SYSTEM,
    ...
    for pred_name, gt_name in node_match_map.items():
        ...
        # Judge states (Llama)
        sj = call_llama(judge_client, STATE_JUDGE_SYSTEM,
    ...
    # ── Pass 3: extract edges using canonical predicted node names (Extraction Model) ───
    print("    Pass 3: edges...")
    ...
    p3 = call_gpt4o(openai_client, PASS3_SYSTEM,
    ...
    # ── Pass 4: CPD matrix for each correctly predicted edge (Extraction Model) ─────────
    ...
    for (gt_parent, gt_child) in correct_edges:
        ...
        p4 = call_gpt4o(openai_client, PASS4_SYSTEM,
        ...
        pred_matrix = p4.get("matrix", [])
        kl_forward = col_kl(pred_matrix, gt_matrix, len(gt_p_states), len(gt_c_states))
        if kl_forward is not None:
            kl_values.append(kl_forward)

            # Compute reverse KL divergence for symmetric metric
            n_pred_parent_states = len(pred_matrix[0]) if pred_matrix else 0
            n_pred_child_states = len(pred_matrix)
            kl_reverse = col_kl(gt_matrix, pred_matrix, n_pred_parent_states, n_pred_child_states)
            if kl_reverse is not None:
                kl_sym = (kl_forward + kl_reverse) / 2.0
                kl_sym_values.append(kl_sym)
                print(f"        {gt_parent}→{gt_child}  KL(GT||Pred)={kl_forward:.4f}  KL(Pred||GT)={kl_reverse:.4f}  KL_sym={kl_sym:.4f}")
            else:
                print(f"        {gt_parent}→{gt_child}  KL={kl_forward:.4f}")
    ...
    print(f"      cpd_kl={result['cpd_kl']}  cpd_kl_symmetric={result.get('cpd_kl_symmetric')}")
...
def write_csv(results, path):
    fields = ["id", "title", "domain", "n_nodes", "n_edges",
              ...
              "cpd_kl", "cpd_kl_symmetric", "n_cpd_evaluated", "error"]
...
def run_result_analysis(results, out_dir="gpt4o_anonymized_result_analysis"):
    ...
    for metric, color, ylabel, title, fname in plot_specs:
        ...
        bxp(ax, gn, kn, metric, color, ylabel, f"{title}  {TAG}", pct=False)
        ax.set_xlabel(X_LABEL, fontsize=10)
        fig.suptitle(f"{title}  (n={nd})  {TAG}", fontsize=12, color="#555", y=1.02)
    ...
    fig, ax = plt.subplots(figsize=(w, 5))
    fig.patch.set_facecolor(BG)
    bxp(ax, gn, kn, "pct_correct_edges", C["edge"],
        "% Correct Connections",
        f"Distribution of % Correct Connections vs Unique Nodes  {TAG}",
        pct=False)
    ax.set_xlabel(X_LABEL, fontsize=10)
    fig.suptitle(f"% Correct Connections  (n={nd})  {TAG}", fontsize=12,
                 color="#555", y=1.02)
    plt.tight_layout(pad=1.2)
    plt.savefig(os.path.join(out_dir, "plot5_pct_correct_edges_vs_nodes.png"),
                dpi=160, bbox_inches="tight", facecolor=BG)
    plt.close()
    print(f"  Saved: {os.path.join(out_dir, 'plot5_pct_correct_edges_vs_nodes.png')}")

    w = max(6, len(kn) * 1.4 + 2)
    fig, ax = plt.subplots(figsize=(w, 5))
    fig.patch.set_facecolor(BG)
    bxp(ax, gn, kn, "cpd_kl", C["cpd"],
        "CPD-KL Divergence  (↓ lower=better)",
        f"Distribution of KL Divergence vs Unique Nodes  {TAG}")
    ax.set_xlabel(X_LABEL, fontsize=10)
    fig.suptitle(f"CPD-KL by Nodes  (n={nd})  {TAG}", fontsize=12,
                 color="#555", y=1.02)
    plt.tight_layout(pad=1.2)
    plt.savefig(os.path.join(out_dir, "plot6_kl_vs_nodes.png"),
                dpi=160, bbox_inches="tight", facecolor=BG)
    plt.close()
    print(f"  Saved: {os.path.join(out_dir, 'plot6_kl_vs_nodes.png')}")

    w = max(6, len(kn) * 1.4 + 2)
    fig, ax = plt.subplots(figsize=(w, 5))
    fig.patch.set_facecolor(BG)
    bxp(ax, gn, kn, "cpd_kl_symmetric", C["cpd"],
        "CPD-KL Symmetric  (↓ lower=better)",
        f"Distribution of Symmetric KL Divergence vs Unique Nodes  {TAG}")
    ax.set_xlabel(X_LABEL, fontsize=10)
    fig.suptitle(f"CPD-KL Symmetric by Nodes  (n={nd})  {TAG}", fontsize=12,
                 color="#555", y=1.02)
    plt.tight_layout(pad=1.2)
    plt.savefig(os.path.join(out_dir, "plot6b_kl_symmetric_vs_nodes.png"),
                dpi=160, bbox_inches="tight", facecolor=BG)
    plt.close()
    print(f"  Saved: {os.path.join(out_dir, 'plot6b_kl_symmetric_vs_nodes.png')}")

    w = max(6, len(kn) * 1.4 + 2)
    fig, ax = plt.subplots(figsize=(w, 5))
    fig.patch.set_facecolor(BG)
    bxp(ax, gn, kn, "pct_spurious_edges", C["edge"],
        "% Spurious Connections",
        f"Distribution of % Spurious Connections vs Unique Nodes  {TAG}",
        pct=False)
    ax.set_xlabel(X_LABEL, fontsize=10)
    fig.suptitle(f"% Spurious Connections  (n={nd})  {TAG}", fontsize=12,
                 color="#555", y=1.02)
    plt.tight_layout(pad=1.2)
    plt.savefig(os.path.join(out_dir, "plot7_pct_spurious_edges_vs_nodes.png"),
                dpi=160, bbox_inches="tight", facecolor=BG)
    plt.close()
    print(f"  Saved: {os.path.join(out_dir, 'plot7_pct_spurious_edges_vs_nodes.png')}")

    w = max(6, len(kn) * 1.4 + 2)
    fig, ax = plt.subplots(figsize=(w, 5))
    fig.patch.set_facecolor(BG)
    bxp(ax, gn, kn, "n_extra_nodes", C["extra"],
        "# Extra (Duplicate) Predicted Nodes",
        f"Extra Predicted Nodes (Duplicates) vs Unique Nodes  {TAG}")
    ax.set_xlabel(X_LABEL, fontsize=10)
    fig.suptitle(f"Extra Nodes (Duplicates)  (n={nd})  {TAG}", fontsize=12,
                 color="#555", y=1.02)
    plt.tight_layout(pad=1.2)
    plt.savefig(os.path.join(out_dir, "plot8_extra_nodes_vs_nodes.png"),
                dpi=160, bbox_inches="tight", facecolor=BG)
    plt.close()
    print(f"  Saved: {os.path.join(out_dir, 'plot8_extra_nodes_vs_nodes.png')}")
...
def main():
    ...
    ap.add_argument("--output-json",     default="gpt4o_gen_llama_eval_results.json")
    ap.add_argument("--output-csv",      default="gpt4o_gen_llama_eval_results.csv")
    ap.add_argument("--output-analysis", default="gpt4o_gen_llama_eval_result_analysis")
    ...
    if not OPENAI_API_KEY:
        print("ERROR: OPENAI_API_KEY environment variable not set"); return
    if not HF_TOKEN:
        print("ERROR: HF_TOKEN environment variable not set"); return

    openai_client = OpenAI(api_key=OPENAI_API_KEY)
    judge_client  = InferenceClient(api_key=HF_TOKEN)
    ...
    if os.path.exists(args.output_json):
        try:
            prev = json.loads(open(args.output_json).read())
    ...
    while True:
        ...
        for sg_id in new_ids:
            ...
            r = process_subgraph(sg_id, sg, openai_client, judge_client)
            results.append(r)
            done_ids.add(sg_id)
            total_processed += 1
            if r.get("error"):
