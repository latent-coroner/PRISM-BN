"""
llm_judge_llama_llama33judge.py — 4-pass Bayesian Network extraction & evaluation
...
Extraction model : Llama 4 Maverick  (via HuggingFace InferenceClient / novita)
...
def call_maverick(client, system, user, max_tokens=2000):
    """Extraction calls — Llama 4 Maverick via novita (requires multimodal content format)."""
...
def call_llama_judge(client, system, user, max_tokens=2000):
...
def eval_pass2_node(judge_result, gt_states, pred_states):
    valid  = [m for m in judge_result.get("state_matches", [])
              if m.get("score", 0) > 0 and "predicted" in m and "ground_truth" in m]
    n_corr = len(valid)
...
def process_subgraph(sg_id, sg, client):
    result = {
    ...
    # ── Pass 1: extract nodes (Llama 4 Maverick) ─────────────────────────────
    print("    Pass 1: nodes...")
    p1 = call_maverick(client, PASS1_SYSTEM, pass1_prompt(text),
    ...
    # Judge nodes (Judge Model)
    nj = call_llama_judge(client, NODE_JUDGE_SYSTEM,
    ...
    # ── Pass 2: extract states (Llama 4 Maverick) ────────────────────────────
    print("    Pass 2: states...")
    ...
    p2 = call_maverick(client, PASS2_SYSTEM,
    ...
    for pred_name, gt_name in node_match_map.items():
        ...
        # Judge states (Judge Model)
        sj = call_llama_judge(client, STATE_JUDGE_SYSTEM,
    ...
    # ── Pass 3: extract edges (Llama 4 Maverick) ─────────────────────────────
    print("    Pass 3: edges...")
    ...
    p3 = call_maverick(client, PASS3_SYSTEM,
    ...
    # ── Pass 4: CPD matrix for each correctly predicted edge (Llama 4 Maverick)
    ...
    for (gt_parent, gt_child) in correct_edges:
        ...
        p4 = call_maverick(client, PASS4_SYSTEM,
...
def run_result_analysis(results, out_dir="llama_maverick_llama33_result_analysis"):
    ...
    TAG = "[Llama4-Maverick + Llama3.3-70B-Judge]"
    ...
    print(f"  [result_analysis] 8 plots + plot_data.json saved (n={nd})")
...
def main():
    ...
    ap.add_argument("--output-json",     default="anonymized_eval_results.json")
    ap.add_argument("--output-analysis", default="anonymized_eval_results")
    ...
    # ── Resume from checkpoint if output JSON already exists ─────────────────
    results = []
    done_ids = set()
    ...
    while True:
        ...
        for sg_id in new_ids:
            ...
            r = process_subgraph(sg_id, sg, client)
            results.append(r)
            done_ids.add(sg_id)
            total_processed += 1
            if r.get("error"):
            ...
            if (total_processed % CHECKPOINT_EVERY) == 0:
                ...
                with open(args.output_json, "w") as f:
                    json.dump({"extraction_model": EXTRACTION_MODEL, "judge_model": JUDGE_MODEL,
                               "aggregate": aggregate(results), "results": results}, f, indent=2)
