"""
4-pass Bayesian Network extraction & evaluation
...
Judge model      : Judge Model (via HuggingFace InferenceClient)
"""
...
def call_qwen(client, system, user, max_tokens=2000):
    """Extraction calls — Qwen via featherless."""
...
def call_llama(client, system, user, max_tokens=2000):
    """Judge calls — Judge Model via Groq."""
    for attempt in range(MAX_RETRIES):
        try:
            completion = client.chat.completions.create(
                model=JUDGE_MODEL,
...
def col_kl(pred_matrix, gt_matrix, n_gt_parent_states, n_gt_child_states):
    ...
    if not pred_matrix or not gt_matrix:
        return None
    nr  = len(pred_matrix)
    ...
    for j in range(n_gt_parent_states):
        if j >= nc:
            break
        gt_col = [float(gt_matrix[r][j]) if r < len(gt_matrix) and gt_matrix[r] and j < len(gt_matrix[r]) and gt_matrix[r][j] is not None else 0.0
                  for r in range(n_gt_child_states)]
        pr_col = [float(pred_matrix[r][j]) if r < len(pred_matrix) and pred_matrix[r] and j < len(pred_matrix[r]) and pred_matrix[r][j] is not None else 0.0
                  for r in range(nr)]
...
def process_subgraph(sg_id, sg, client):
    ...
    # ── Pass 1: extract nodes (Qwen) ─────────────────────────────────────────
    print("    Pass 1: nodes...")
    p1 = call_qwen(client, PASS1_SYSTEM, pass1_prompt(text),
    ...
    p2 = call_qwen(client, PASS2_SYSTEM,
    ...
    p3 = call_qwen(client, PASS3_SYSTEM,
    ...
    # ── Pass 4: CPD matrix for each correctly predicted edge (Qwen) ───────────
    ...
    for (gt_parent, gt_child) in correct_edges:
        ...
        p4 = call_qwen(client, PASS4_SYSTEM,
...
def run_result_analysis(results, out_dir="qwen_anonymized_result_analysis"):
    ...
    bxp(ax, gn, kn, "cpd_kl_symmetric", C["cpd"],
        "CPD-KL-Symmetric Divergence  (↓ lower=better)",
